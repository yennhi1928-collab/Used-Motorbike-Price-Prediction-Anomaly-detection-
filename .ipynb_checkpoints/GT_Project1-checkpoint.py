...
# =========================
st.set_page_config(
    page_title="Used Motorbike Price Prediction & Abnormal Price Detection",
    layout="centered"
)

# ==== CSS PASTEL & ICON STYLE ====
st.markdown(
    """
    <style>
    /* Nền tổng thể */
...
✔ Làm mềm và đẹp toàn bộ giao diện
✔ Tùy chỉnh được sidebar, nút bấm, header
✔ Làm bảng dữ liệu đẹp hơn
✔ Tạo cảm giác ứng dụng chuyên nghiệp, hiện đại
...
st.markdown(
    ...
    ">
        <h1 style="color:#111827; margin:0; font-size: 1.9rem;">
            🛵 Used Motorbike Price Prediction & Abnormal Price Detection
        </h1>
    </div>
    """,
    unsafe_allow_html=True
...
def detect_anomalies(df, model, threshold=50, method='absolute'):
    """
    df: DataFrame đã có cột 'gia' (giá thực tế)
    model: pipeline RandomForestRegressor đã train
    """
    df = df.copy()

    # Dự đoán giá từ mô hình đã huấn luyện
    feature_cols = [c for c in df.columns if c != 'gia']
...
# ---------- 4. Detecting Abnormal Listings ----------
    ...
    pastel_header("🚨", "Detecting Abnormal Listings", "#ede9fe")
    ...
    if (df_processed is None) or (model_default is None):
        ...
        if st.button("Anomaly checking"):
            try:
                ...
                new_row = df_all_anom.iloc[-1]  # dòng cuối là tin mới
...
- **Member 1:** Nguyễn Thị Xuân Mai  
  - Email: nguyentxmai@gmail.com  
  - Role: GUI for Project 1 – Motorbike price prediction and anomaly detection  

- **Member 2:** Trần Thị Yến Nhi  
  - Email: yennhi1928@gmail.com  
  - Role: GUI for Project 2 – Content-based similar motorbike recommendation and motorbike clustering  
""")